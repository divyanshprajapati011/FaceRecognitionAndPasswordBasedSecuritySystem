...
#     if logs:
#         df = pd.DataFrame(logs[-50:][::-1])  # Last 50 logs
...
#         tab1, tab2, tab3 = st.tabs(["👥 Manage Users", "📊 Logs Analytics", "⚙️ System"])
...
#                     with col3:
#                         if st.button("🔄 Retrain", key=f"train_{username}"):
#                             if train_model():
#                                 st.success("✅ Model retrained!")
#                             else:
#                                 st.error("❌ Training failed!")
                
#                 if st.button("🧹 Delete All Users & Reset"):
...
#                 # Filter dataframe
#                 df_filtered = df[df['status'].str.contains('|'.join(status_filter), na=False)]
...
#                     hourly = df_filtered.groupby(df_filtered['time'].dt.hour).size()
...
#             with col3:
#                 st.metric("Model Status", "✅ Ready" if os.path.exists("trainer.yml") else "❌ Train Needed")
            
#             st.info(f"**Admin Password**: `{ADMIN_PASSWORD}` (Change in code!)")
...
#     else:
#         st.warning("⚠️ Enter correct admin password")
#         if st.button("Login"):
#             if admin_password == ADMIN_PASSWORD:
#                 st.success("✅ Admin Access Granted!")
#                 # st.balloons()
